        ...
        try:
            ...
            if raw_data:
                ...
                data_dict = {
                    ...
                    'Angle': [],
                    'Status': [],
                      # Add Angle column here
                }

                # Get the parameter data dynamically based on part_model
                parameter_data = paraTableData.objects.filter(
                    parameter_settings__part_model=part_model
                ).values('parameter_name', 'usl', 'lsl')

                # Add parameter data columns to the dictionary
                ...
                for record in filtered_data:
                    ...
                    if date not in grouped_data:
                        grouped_data[date] = {
                            ...
                            'Parameters': {key: set() for key in data_dict if key not in ['Date', 'Shift', 'Operator', 'Status', 'Job Numbers', 'Angle']},
                            'Angle': record.angleValue,  # Store the angleValue here
                        }
                    ...
                    # Add parameter data for each record
                    for param in parameter_data:
                        param_name = param['parameter_name']
                        key = f"{param_name} <br>{param['usl']} <br>{param['lsl']}"
                ...
                for date, group in grouped_data.items():
                    ...
                    data_dict['Status'].append(group['Status'] if group['Status'] else '')
                    data_dict['Angle'].append(group['Angle'] if group['Angle'] else '')  # Ensure Angle is added

                    # Add parameters to the dictionary
                    for key, values in group['Parameters'].items():
                        # Join the parameter values or append empty string if no values
                        data_dict[key].append("<br>".join(sorted(values)) if values else '')
